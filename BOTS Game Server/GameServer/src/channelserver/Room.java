...
public class Room {
	...
	private final String[] roomplayer = new String[8]; // names of players in
														// room
	private final PrintWriter[] roomsocks = new PrintWriter[8]; // sockets of
																// players in
																// room
	...
	private final boolean[] roomready = new boolean[8]; // ready players in the
														// lobby room
	...
	public Room(int[] rnum, String rname, String rpass, int rlvl, String rowner, String rip, PrintWriter osock,
		...
		try {
			...
			pack.addByte4((byte) Integer.parseInt(aip[0]), (byte) Integer.parseInt(aip[1]),
	...
	private Packet getRoomPacket() {
		...
		packet.addByte2(this.roomtype, (byte) 0x08);
	...
	public void monsterKilled(int typ, int num, int killedby) {
		try {
			...
			if (this.monster[num] == typ) {
				...
				// if(typ == 2) {
				// packet.addByte4((byte) 0x25, (byte) 0x2F, (byte) 0x08,
				// (byte)0x00);
				// } else if (typ == 102)
				// {
				...
				// }
				// else
				// packet.addByte4((byte) 0x01, (byte) 0x00, (byte) 0x0C, (byte)
				// 0x06);
